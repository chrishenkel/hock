...
var Hock = module.exports = function (options) {
  options = options || {};
  this._failOnUnmatched = (typeof options.failOnUnmatched === 'boolean' ? options.failOnUnmatched : true);
...
Hock.prototype.done = function (cb) {
  ...
  if (this._unMatched.length) {
    err = new Error('Unmatched Requests: \n'
    + JSON.stringify(this._unMatched.map(function (req) {
 ...
 * @returns {Request}
 */
Hock.prototype.options = function (url, body, headers) {
  return new Request(this, {
    method: 'OPTIONS',
    url: url,
    body: body || '',
